(()=>{
  ...
  const championTeams={
    2019:'Turn Down for WATT',
    2020:"Mott's Applesauce",
    2021:"Mott's Applesauce",
    2022:'The Collector',
    2023:'Team Bad fingers',
    2024:'The Juggernaut',
    2025:'The Injured Reserve'
  };
  const champs=document.getElementById('champions');
  if(champs){
    champs.innerHTML=DATA.champions.map(c=>`<div class="champ"><small>${c.year} Champion</small><strong>${esc(c.manager)}</strong><span class="champ-team">${esc(championTeams[c.year]||'')}</span></div>`).join('');
  }

  const historyMetrics={
    legacy:{label:'Legacy Score',get:m=>DATA.legacy[m]?.score??0,fmt:(m,v)=>Number(v).toLocaleString(),sort:(a,b)=>(DATA.legacy[b]?.score??0)-(DATA.legacy[a]?.score??0),sub:m=>`${DATA.regular[m]?.total||'—'} career`},
    wins:{label:'Reg. Wins',get:m=>recWins(DATA.regular[m]?.total),fmt:(m,v)=>String(v),sort:(a,b)=>recWins(DATA.regular[b]?.total)-recWins(DATA.regular[a]?.total),sub:m=>`${(DATA.regular[m]?.winPct*100).toFixed(1)}% win rate`},
    pct:{label:'Win %',get:m=>DATA.regular[m]?.winPct??0,fmt:(m,v)=>(v*100).toFixed(1)+'%',sort:(a,b)=>(DATA.regular[b]?.winPct??0)-(DATA.regular[a]?.winPct??0),sub:m=>DATA.regular[m]?.total||'—'},
    avg:{label:'Avg Finish',get:m=>Math.max(0,13-(DATA.legacy[m]?.avg??13)),fmt:m=>Number.isFinite(DATA.legacy[m]?.avg)?DATA.legacy[m].avg.toFixed(2):'—',sort:(a,b)=>(DATA.legacy[a]?.avg??99)-(DATA.legacy[b]?.avg??99),sub:m=>`${DATA.legacy[m]?.titles||0} title${DATA.legacy[m]?.titles===1?'':'s'}`}
  };
  ...
  function renderHistory(){
    ...
    historyChart.innerHTML=managers.map((m,i)=>{
      const v=metric.get(m),width=Math.max(3,v/max*100),status=current.has(m)?'Active':'Former';
      return `<div class="archive-bar"><div><strong>${i+1}. ${esc(m)}</strong><small>${esc(metric.sub(m))} · ${status}</small></div><div class="archive-track"><div class="archive-fill" style="width:${width}%"></div></div><div class="archive-value">${esc(metric.fmt(m,v))}</div></div>`;
    }).join('');
  }
  renderHistory();
  ...
  function renderTimeline(){
    ...
    if(!years.length){timelineBox.innerHTML='<div class="timeline-empty">No season data for this metric.</div>';return}
