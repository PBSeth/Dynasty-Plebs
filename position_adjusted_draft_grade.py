...
new=r'''function renderDraftIntel(){
 const box=document.getElementById('draftIntel'),allManagers=Object.keys(rookiePicks),getScored=m=>(rookiePicks[m]||[]).filter(p=>Number.isFinite(p.ppg)&&p.pos),scored=getScored(selectedManager);
 ...
 const roundCards=[1,2,3,4].map(r=>{const d=scored.filter(p=>bucket(p)===r),raw=rawAvg(d),adj=adjAvg(d,selectedManager),label=r===4?'Round 4+':`Round ${r}`,ranking=rank(adj,managerMetrics.map(x=>x.rounds[r]));return`<div class="intel-card"><small>${label} Avg Career PPG</small><b>${raw==null?'—':raw.toFixed(1)}</b><strong>${ranking||'No scored picks'}${Number.isFinite(adj)?` · ${adj>=0?'+':''}${adj.toFixed(1)} adj`:''}</strong><span>${d.length} scored pick${d.length===1?'':'s'}</span></div>`}).join('');
 box.innerHTML=`<div class="intel-card"><small>Avg Career PPG / Pick</small><b>${me.avg.toFixed(1)}</b><strong>Raw scoring only</strong><span>${me.n} scored picks</span></div><div class="intel-card"><small>Pos + Round Draft Value</small><b>${me.adj==null?'—':(me.adj>=0?'+':'')+me.adj.toFixed(1)}</b><strong>${rank(me.adj,managerMetrics.map(x=>x.adj))}</strong><span>vs other managers at same position + round</span></div><div class="intel-card"><small>Best Adjusted Pick</small><strong>${best?best.player:'—'}</strong><span>${best?`${best.year} · ${best.pick} · ${best.ppg.toFixed(1)} PPG · ${best.adj>=0?'+':''}${best.adj.toFixed(1)} adj`:'No scored picks'}</span></div><div class="intel-card"><small>Worst Adjusted Pick</small><strong>${worst?worst.player:'—'}</strong><span>${worst?`${worst.year} · ${worst.pick} · ${worst.ppg.toFixed(1)} PPG · ${worst.adj>=0?'+':''}${worst.adj.toFixed(1)} adj`:'No scored picks'}</span></div>${roundCards}<div class="intel-note">Career PPG remains the raw descriptive stat. Manager draft grading is position- and round-adjusted: each pick is compared with other managers' picks at the same position and rookie-draft round, preventing QB scoring from inflating a manager's grade. Sparse position/round groups fall back to same-position peers, then same-round peers. New NFL seasons roll into career PPG and automatically update the adjusted values.</div>`;
}'''
...
p.write_text(s2)
print('Applied position + round adjusted draft grading')
